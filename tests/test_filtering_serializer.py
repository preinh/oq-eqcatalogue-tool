# Copyright (c) 2010-2012, GEM Foundation.
#
# eqcatalogueTool is free software: you can redistribute it and/or modify it
# under the terms of the GNU Affero General Public License as published
# by the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# eqcatalogueTool is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
# GNU General Public License for more details.
#
# You should have received a copy of the GNU Affero General Public License
# along with eqcatalogueTool. If not, see <http://www.gnu.org/licenses/>.

import tempfile
import unittest
from eqcatalogue import filtering
from eqcatalogue.serializers import csv

from tests import test_utils


class ShouldExportMeasures(unittest.TestCase):
    EXPECTED_CSV_1 = """
"""

    EXPECTED_CSV_2 = """
"""

    def setUp(self):
        test_utils.load_catalog()

    def test_measures_to_csv(self):
        csv_file = tempfile.NamedTemporaryFile()
        filtering.C(agency__in=['ISC']).export(
            'csv', filename=csv_file.name, mode="a")
        self.assertEqual(self.EXPECTED_CSV_1, csv_file.read())
        csv_file.close()

        csv_file = tempfile.NamedTemporaryFile()
        filtering.C(agency__in=['IDC']).export(
            'csv', filename=csv_file.name, mode="a")
        self.assertEqual(self.EXPECTED_CSV_2, csv_file.read())

    def test_csv_serializers(self):
        csv_file = tempfile.NamedTemporaryFile()
        measures = filtering.C(agency__in=['ISC'])

        csv.export_measures(measures, filename=csv_file.name, mode="a")
        self.assertEqual(self.EXPECTED_CSV_1, csv_file.read())
        csv_file.close()

        measures = filtering.C(agency__in=['IDC'])
        csv.export_measures(measures, filename=csv_file.name, mode="a")

        self.assertEqual(self.EXPECTED_CSV_2, csv_file.read())
